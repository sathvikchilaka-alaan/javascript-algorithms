class ColorSwatch {
    constructor(presets={}, defaultPreviewColor = [255, 255, 255], alpha = 0.1) {
        ...
        this.previewNearestHexCode = document.getElementById("preview-nearest-color-hex")
    ...
    init(defaultPreviewColor) {
        ...
        if(Object.keys(this.presets).length === 0) this.presets = this.getComputedStylesFromCSS()
        ...
        if(this.swatchesEl){
            this.swatchesEl.addEventListener('click', (e)=>{
                ...
                this.applyRGBColors(this.presets[swatchColor][0], this.presets[swatchColor][1], this.presets[swatchColor][2])
            })
    ...
    parseRgbStringToArray(colorStr) {
        ...
        const inside = colorStr.slice(colorStr.indexOf("(") + 1, colorStr.lastIndexOf(")"))
    ...
    compressHexCodeIfPossible(hex){
        ...
        const [ x, y, z] = [ hexCode.slice(0, 2), hexCode.slice(2, 4), hexCode.slice(4, 6)]
    ...
    updateHexValueforRGB() {
        this.previewHexCode.textContent = this.compressHexCodeIfPossible(this.currentHexCode)
        this.previewNearestHexCode.textContent = this.getNearestHexCode(this.currentColor)
    }

    updatePreviewTextColor = () => {
        const isDark = (this.currentColor[0] + this.currentColor[1] + this.currentColor[2]) < (256*3)/2
