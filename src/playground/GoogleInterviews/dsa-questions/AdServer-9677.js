...
class AdsCenterBasic{
    ...
    getAd(){
        ...
        if(this.lastServedAd){
            ...
            if(this.lastServedAd.score > 0) this.availableAds.push(this.lastServedAd)
        }
...
class AdsCenterFollowUp1{
    constructor(){
        ...
        this.cooldownAds = new BinaryHeap((a,b)=> a.nextAvailableTime < b.nextAvailableTime)
    }

    insertAd(content, score, delay=1){ // Delay=1, coz we still dont want to have consecutive Ads
        const newAd = {
            id: this.idCounter++,
            content,
            score,
    ...
    getAd(){
        ...
        while(this.cooldownAds.size() > 0 && this.cooldownAds.peek().nextAvailableTime <= this.timeCompleted){
...
class AdsCenterFollowUp2{
    ...
    insertAd(content, score){ // No Delay needed, as we add N nextAvailableTime to cooldown items
        const newAd = {
            id: this.idCounter++,
            content,
            score
